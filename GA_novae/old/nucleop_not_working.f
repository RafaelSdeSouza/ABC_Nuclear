...
C    calculation if ILAST=3 is selected; normally we get [see Longland, AA 548, 
C    A30 (2012)]:
C
...
ccc   convert nucleo to subroutine
      subroutine nucleop(parm,x,ciso) 

c     X: mass fractions of all nucldies in network
...
ccc   define new vector with running parameters
      real*8 parm(4)
      character*5 ciso(nsp)

ccc   running parameters from nucleo.in            
      COMMON/NUCINP/nurin,nucin,nuchar
      
ccc      inputfile='nucleo.in'
ccc      open(98,file=inputfile,status='unknown')
...
c     if the step failed, return for another try
 ...
 1275    CONTINUE
      ...
      IF((NSTEP.GE.NLAST).OR.(T.GE.TLAST).or.(X(ISF).LE.XLAST)) THEN        
         NT=NT+1                                ! end of run
ccc         CALL STOCK(NT,T,SUMX,X,XST)            ! stocking the last values
ccc         CALL SEXIT(NT,T,X,XST)                 ! exit run
...
ccc      OPEN(9,FILE='nucleo.out',status='unknown')     
ccc      OPEN(21,FILE='totflux.out',STATUS='unknown')
ccc      OPEN(22,FILE='instantflux.out',STATUS='unknown')
...
ccc      if(((itest.eq.1).and.(ilast.eq.3)))then
ccc         write(6,*) 'unacceptable running parameters:'
ccc         write(6,*) 'check ILAST, ITEST etc.'
ccc         write(6,*)
ccc         stop
ccc      endif
...
ccc         insert parameters here, but keep running time from nucleo.in
            TAU0=parm(1)
	           ...
	           scale1=parm(3)
            scale2=parm(4)
ccc            open(26,file='profile.out',status='unknown')
ccc            write(26,9119)
         ENDIF
...
ccc      ENDIF
C------------------------------------
ccc      IF(INMODE.EQ.2) THEN
ccc         CALL PROFIL(T)   ! calculate initial TAU, RHO from profile
ccc         if(ilast.eq.0) then
ccc            write(9,9300)
ccc         endif	 
...
ccc         call searchmodel(t)  ! calculate initial shell properties
ccc         if(ilast.eq.0) then
ccc            write(9,9300)
ccc         endif	 
...
ccc 696    continue
ccc 315    format(' ',' Reaction ',i4,' changed by a factor of ',1PD10.3)
ccc        write(9,*)
ccc      endif
...
ccc      if(inmode.eq.2) then
ccc         deltaprof=(tauprofmax-tauprocalmax)/tauprofmax
...
ccc      if((abs(xavsum-1.d0)).gt.0.001) then
ccc         write(6,*) 'WARNING: AVERAGED MASS FRACTION SUM IS NOT 1!'
ccc         stop
ccc      endif
