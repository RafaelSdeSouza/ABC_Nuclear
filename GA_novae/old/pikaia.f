c
c     Changes by Christian Iliadis [08/01/2019]
c
c     - format of output
c     - output to file
c     - increased some parameter settings
...
c     Constants
      ...
      parameter (NMAX = 32, PMAX = 5000, DMAX = 20)
c
...
c
c     Output: none
      open(99,file='main.dat',status='unknown')
...
c         write(*,'(/i6,i6,f10.6,4f10.6)') ig,nnew,pmut,
...
c         write(9,9000) ig,fitns(ifit(np/2)),
c     &       (nint(ndpwr*oldph(k,ifit(np/2))),k=1,n)

c        output results for fittest individual:
         write(99,9000) ig,fitns(ifit(np)),
     &      (nint(ndpwr*oldph(k,ifit(np))),k=1,n)

      endif
      
 9000 format(i6,2x,f10.6,2x,10i12)
      return
      end
