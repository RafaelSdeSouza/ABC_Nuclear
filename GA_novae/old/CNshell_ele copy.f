...
c     ===================================================             |
      program CNshell_ele
c
c     NOVA ELEMENT NUCLEOSYNTHESIS
c
c     03/08/2019; author: Christian Iliadis
c
c     script assumes INMODE=3 [exponentially decaying profile];
c     it samples:
c
...
c       script as: X(16O) = 1 - X(12C) - X(20Ne) - ...
...
c
c     the script checks if ILAST=3 [MC option] and INMODE=3 
c     [exponentially decaying profile] are set, otherwise run stops
c
c     NOTE 1: nucleo.dat must list SOLAR ABUNDANCES! Otherwise, run stops
c
c     NOTE 2: running time is now set in CNshell_ele.dat, not nucleo.in
c
c     INPUT FILE: CNshell_ele.dat 
c        - exponential profile parameter sampling ranges
c        - WD abundance sampling ranges
c        - mixing between outer WD and accreted solar matter
c        - running time tlast
c        - atomic number, Z, and mass fractions of all observed elements;
c          negative mass fractions are interpreted as upper limits; 
c          rows that start with '!' are disregarded as input; 
c          INPUT IS ALWAYS MASS FRACTIONS 
c        - if iflag.ne.0: script analyzes and compares mass fraction
c          ratio, x(Z_i)/X(Z=iflag)
c
c     OUTPUT
c     * only if simulated and observed abundances agree for all observed
c     * elements within a factor of 10:
c
c     CNshell_ele1.out: simulated elemental mass fractions or mass fraction
c                       relative to X(Z), depending on iflag=Z in input
c     CNshell_ele2.out: sampled parameters 
c
c     * only if simulated and observed abundances agree for all observed
c     * elements within user-defined factor, uf:
c
c     CNshell_ele3.out: sampled isotopic abundances and reaction rate
c                       variation factors
c     CNshell_ele4.out: output of intermediate results, for testing
c
...
c   VARIABLES
      ...
      integer ilast,inmode,zexp(100),nxx,non,neg,negh
...
c     xexp: oberved abundance; xx: simulated abundance        
      ...
      real*8 xwd_c12_min,xwd_c12_max          
      ...
      real*8 ratio_max,ratio_new,tlast,xx_norm
...
c=======================================================================
c   DATA INPUT      
      open(601, file='CNshell_ele.dat',status='unknown')
...
c     input is always mass fractions
c     iflag=0: analysis of mass fractions 
c     iflag=Z: analysis of mass fractions relative to X(Z); Z: atomic number
      read(601,'(i2)') iflag
      read(601,6000) string
      read(601,6000) string
...
c     read isotope name and experimental mass fraction
 ...
 61   read(601,*,end=60) charz,xexp(i)
      if(charz(1:1).eq.'!')then
          goto 61
       else
         read(charz(1:3),'(i3)') zexp(i)
         i=i+1
         goto 61
      endif
 ...
 60   continue
      ...
      neg=0    ! counter for sampled negative oxygen abundance
      non=0    ! counter for successful completion of network
      negh=0   ! counter for negative abundance of standard
...
c     stop run if both f_min and f_max are zero
      if((f_min.eq.0.d0).and.(f_max.eq.0.d0))then
         write(6,*) 'Mixing factor is zero. Run stop!'
         stop
      endif
...
c   USER CONSOLE INPUT
      write(6,*)
      write(6,*) 'ALWAYS INPUT MASS FRACTIONS IN .dat FILE'
      write(6,*)
...
c     for simulated elemental abundances [all solutions]
      open(701,file='CNshell_ele1.out',status='unknown') 
      write(701,6000) string3
      write(701,*) 'total # of samples:',nsamp
      write(701,*) '       random seed:',nseed
      write(701,*) 'sample thermonuclear rates: ',choice
      if(iflag.eq.0)then
         write(701,*) 'output: mass fractions'
       else
         write(701,6004) iflag
      endif
      write(701,6002) (zexp(j),j=1,nexp)     

c     for simulated profile parameters [all solutions]
      open(710,file='CNshell_ele2.out',status='unknown') 
      write(710,6000) string3
      write(710,*) 'total # of samples:',nsamp
      write(710,*) '       random seed:',nseed
      write(710,*) 'sample thermonuclear rates: ',choice
      write(710,6003)

c     for reaction rate variation factors [choice='y' only]
      open(720,file='CNshell_ele3.out',status='unknown') 
...
c     other output [for testing, etc.]
      open(740,file='CNshell_ele4.out',status='unknown')
...
c     INMODE=3 for run with exponential profile
 ...
 79   continue
      if((index(string,'!').eq.0))then
         write(6,*) 'Check input: nuclei.in and nucleo.dat!'
         stop
      endif
      close(702) 
...
c        sample WD outer core mass fractions
...
c        oxygen abundance given by 1 minus the other abundances
     ...
     &               -xwd_mg24-xwd_mg25-xwd_mg26-xwd_al27
         ...
         if(xwd_o16.lt.0.d0)then
c           next run
            goto 300
         endif

c        sample mixing fractions
         lgf=lgf_min+(lgf_max-lgf_min)*ran1(nseed)
         ff=10.d0**lgf

c        ===============================================================
c        ===============================================================
c        enter sampled profile parameters into nucleo.in file 
...
c        read nucleo_shell.in and construct new nucleo.in
 ...
 102     continue
         read(802,*) pro1,pro2,pro3,pro4,pro5   ! tlast: running time
         write(801,1100) temp,rhop,tlast,timetem,timerho
...
c        enter sampled abundances into nucleo.dat file
...
c        normalize all solar mass fractions from nucleo.dat; then
c        mix abundances: [1 part WD with ff parts solar],
...
c           read iso abundances for later output
            do 107 j=1,2
...
c        =============================================================
c        if iflag.ne.0: mass fraction ratios [i.e., relative to X(iflag)]
c        will be analyzed
c        =============================================================
         if(iflag.ne.0)then
...
c           - divide all experimental elemental abundances by xexp(iflag)
            do 112 jj=1,nexp
                  xexp(jj)=xexp(jj)/xx_norm
 112        continue

c           - divide all computed elemental abundances by xx(iflag)
            xx_norm=xx(iflag)
...
c        output intermediate results:
         write(740,3000) k,temp
         write(740,3004)
c        determine maximum ratio of observed and computed abundances
         ratio_max=1.0d0
         
         do 113 j=1,nexp
            if((xexp(j).gt.0.d0).and.(xx(zexp(j)).gt.1.d-30))then  ! no upper limit input
               ratio_new=xx(zexp(j))/xexp(j)
               if(ratio_new.le.1.0d0)then
c                 inverse to compare factors >1
                  ratio_new=1.d0/ratio_new     
               endif
               write(740,3001) zexp(j),xexp(j),xx(zexp(j)),ratio_new
c            upper limit input [negative value]
             elseif((xexp(j).lt.0.d0).and.(xx(zexp(j)).gt.1.d-30))then                           
               ratio_new=xx(zexp(j))/dabs(xexp(j))
c              simulation agrees with upper limit, nothing else can 
c              be concluded
               if(ratio_new.le.1.0d0)then   
                  ratio_new=1.0d0
               endif
               write(740,3001) zexp(j),xexp(j),xx(zexp(j)),ratio_new               
c            if simulated abundances become negligible or negative 
             elseif(xx(zexp(j)).le.1.d-30)then
               ratio_new=1.0d99 
               write(740,3001) zexp(j),xexp(j),xx(zexp(j)),ratio_new
             else  
               write(6,*) 'Find out why program stopped...'
               stop
            endif
...
c           elemental abundances 
            write(701,6001) k, (xx(zexp(j)),j=1,nexp)

c           profile parameters
            write(710,1101) temp,rhop,trun,timetem,timerho,
...
c        if network does not complete or no solution is obtained
         if(lll.eq.0)then
            non=non+1
            close(707)      ! close nucleo.out and prepare for next run
          elseif(xx_norm.lt.0.d0)then
            negh=negh+1            
         endif
...
c        replace temporary nucleo.dat by help file
 ...
 300     continue 
c        if sampled oxygen abundance is negative
         if(xwd_o16.le.0.d0)then
            neg=neg+1
         endif
...
c=======================================================================
      ...
      write(740,*)      
      write(740,6005) (real(non)/real(nsamp))*100.d0
      write(740,6006) (real(neg)/real(nsamp))*100.d0
      write(740,6007) (real(negh)/real(nsamp))*100.d0
      write(6,*)      
      write(6,6005) (real(non)/real(nsamp))*100.d0
      write(6,6006) (real(neg)/real(nsamp))*100.d0
      write(6,6007) (real(negh)/real(nsamp))*100.d0
...
c   FORMATS
 ...
 1101 format(9(1pe9.2e2,2x))
 3000 format('Run # ',i5,5x,'Tpeak=',1pe10.2e3)
 3001 format(i5,2x,3(1pe10.2e3,2x))
 3002 format('Maximum ratio=',1pe10.2e3)
 3004 format(4x,'Z',6x,'obs',9x,'sim',6x,'|obs/sim|')
 6000 format(a100)
 6001 format(i5,2x,25(1pe10.2e3,2x))
 6002 format(' Run',8x,25(i2,10x))
 6003 format(2x,'T_peak',4x,'rho_peak',3x,'time_run',4x,'tau_T',
     &       5x,'tau_rho',4x,'xwd_c12',4x,'xwd_o16',6x,'ff',7x,
     &       'ratio_max')
 6004 format(' output: mass fractions relative to Z=',i2)
 6005 format(' fraction of runs that did not complete:',f6.1,'%')
 6006 format(' fraction of runs with sampled X(O) < 0:',f6.1,'%')
 6007 format(' fraction of runs with sampled X(iflag) < 0:',f6.1,'%')
