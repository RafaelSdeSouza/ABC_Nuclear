...
simu <- read.table("main.out", header=FALSE, skip=3, nrows=9, 
           na.strings = "NA")
# read parameter bounds
bounds <- read.table("main.in", header=FALSE,  sep=",", skip=16, nrows=9,
...
################ PLOT lt 
plot( 1, type="n", lwd=2, xlim=c(150,300), ylim=c(50,9.0e3), 
...
title(xlab=expression(paste(T (MK))), line=2.0, cex.lab=1.3)  
title(ylab=expression(paste(rho, " ",(g/cm^3))), line=2.5, cex.lab=1.3)
...
################ PLOT lb 
plot( 1, type="n", lwd=2, xlim=c(100,1.0e4), ylim=c(10,1.0e3),
       main="", xlab = "", ylab = "", 
...
# control distance between axis and label [line=...]
title(xlab=expression(paste(tau[T], " ", (s))), line=1.9, cex.lab=1.3)
title(ylab=expression(paste(tau[rho], " ", (s))), line=2.5, cex.lab=1.3)
...
plot( 1, type="n", lwd=2, xlim=c(0.4, 10.0), ylim=c(0.1, 1), 
       main="", xlab = "", ylab = "", 
       cex=1.0, cex.lab=1.0, cex.main=1.0, cex.axis=1.0, log="xy", axes=FALSE )
# control distance tick mark labels and axis
# don't touch first number
...
title(ylab=expression(paste("X"[WD],"(",NULL^"12","C",")")), 
        line=1.9, cex.lab=1.3)
title(xlab=expression(paste(Mixing," ",f)), line=2.0, cex.lab=1.3)

wdc12 <- simu[5,]
points(simu[9,], wdc12, col="red", pch=16, cex=2)

abline(v=3)
abline(h=0.5)

################ PLOT rb 
## ratio of WD 16C/20Ne versus mixing fraction f
plot( 1, type="n", lwd=2, xlim=c(0.4,10.0), ylim=c(0.1,9.0),
       main="", xlab = "", ylab = "", 
       cex=1.0, cex.lab=1.0, cex.main=1.0, cex.axis=1.0, log="xy", axes=FALSE )
# control distance tick mark labels and axis
# don't touch first number
...
# control distance between axis and label [line=...]
title(xlab=expression(paste("X"[WD],"(",NULL^"12","C",")","/","X"[WD],"(",NULL^"16","O",")")), line=2.0, cex.lab=1.3)
title(ylab=expression(paste("X"[WD],"(",NULL^"16","O",")","/","X"[WD],"(",NULL^"20","Ne",")")), 
       line=1.9, cex.lab=1.3)

wdc12 <- simu[5,]
wdo16 <- 1 - simu[5,] - simu[6,] - simu[7,] - simu[8,]
wdne20 <- simu[6,]

points(wdc12/wdo16, wdo16/wdne20, col="red", pch=16, cex=2)
