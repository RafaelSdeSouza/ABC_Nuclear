...
c     References:
c
c        Charbonneau, Paul. "An introduction to genetic algorithms for
...
c
c     Output: none
      open(9,file='main.dat',status='unknown')
...
c        output results for fittest individual:
         write(9,9000) ig,fitns(ifit(np)),
     &      (nint(ndpwr*oldph(k,ifit(np))),k=1,n)

      endif
      
 9000 format(i6,2x,1pe11.3e3,2x,10i12)
      return
      end
